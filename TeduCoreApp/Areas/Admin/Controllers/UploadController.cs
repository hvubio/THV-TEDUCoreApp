    ...
    public class UploadController : BaseController
    {
        ...
        public async Task UploadImageForCkEditor(IList<IFormFile> upload, string CKEditorFuncNum, string CKEditor, string langCode)
        {
            ...
            else
            {
                ...
                var query = "<script>window.parent.CKEDITOR.tools.callFunction( " + CKEditorFuncNum + ", '" +
                               Path.Combine(imageFolder, filename).Replace(@"\", @"/") + "');</script>";
                await HttpContext.Response.WriteAsync(query);
            }
        }
        /// <summary>
